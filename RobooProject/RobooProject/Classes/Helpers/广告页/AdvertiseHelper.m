...
//    NSString *imageName = stringArr.lastObject;
    NSString *imageName = [imageUrl substringFromIndex:imageUrl.length-14];
...
{
    NSString *url = @"https://github.com/bboyluobo";
    if (!RBIsEmpty(url)) {
        if([[UIApplication sharedApplication] canOpenURL:[NSURL URLWithString:url]]) {
            [[UIApplication sharedApplication] openURL:[NSURL URLWithString:url] options:@{@"username" : @"roobo"} completionHandler:^(BOOL success) {
                NSLog(@"%zd", success);
            }];
        }
    }
