...
- (void)viewDidLoad {
    ...
    RBWeak(self);
    [self.webView addObserverBlockForKeyPath:RBKeyPath(weakself.webView, estimatedProgress) block:^(id  _Nonnull obj, id  _Nullable oldVal, id  _Nullable newVal) {
    ...
    [self.webView addObserverBlockForKeyPath:RBKeyPath(self.webView, title) block:^(id  _Nonnull obj, id  _Nullable oldVal, id  _Nullable newVal) {

        if (!RBIsEmpty(newVal) && [newVal isKindOfClass:[NSString class]] && [self webViewController:self webViewIsNeedAutoTitle:self.webView]) {
    ...
    [self.webView.scrollView addObserverBlockForKeyPath:RBKeyPath(self.webView.scrollView, contentSize) block:^(id  _Nonnull obj, id  _Nullable oldVal, id  _Nullable newVal) {
    ...
    }else if (!RBIsEmpty(self.contentHTML)) {
...
- (UIView *)rbNavigationBarLeftView:(RBNavigationBar *)navigationBar
{
    ...
    self.closeBtn.rb_x = leftView.rb_width - self.closeBtn.rb_width;
...
#pragma mark - RBWebViewControllerDataSource
// 默认需要, 是否需要进度条
...
#pragma mark - RBWebViewControllerDelegate
// 导航条左边的返回按钮的点击
...
- (UIProgressView *)progressView
{
    if(_progressView == nil && [self.parentViewController isKindOfClass:[UINavigationController class]])
    {
        ...
        progressView.rb_height = 1;

        progressView.rb_width = kScreenWidth;

        progressView.rb_y = self.rb_navgationBar.rb_height;
...
- (void)dealloc
{
    NSLog(@"RBWebViewController -- dealloc");
