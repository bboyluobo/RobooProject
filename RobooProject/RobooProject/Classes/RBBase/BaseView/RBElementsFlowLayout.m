...
@implementation RBElementsFlowLayout
...
- (void)prepareLayout
{
    ...
    //如果重新刷新就需要移除之前存储的高度
    //复赋值以顶部的高度, 并且根据列数
    self.rb_LastAtrbsFrame = CGRectMake(0, 0, self.collectionView.frame.size.width, 0);
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    CGFloat rightLeftWidth = self.collectionView.frame.size.width - CGRectGetMaxX(self.rb_LastAtrbsFrame) - [self xMarginAtIndexPath:indexPath] - self.edgeInsets.right;
    ...
    if (rightLeftWidth >= w) {
        ...
        x = CGRectGetMaxX(self.rb_LastAtrbsFrame) + [self xMarginAtIndexPath:indexPath];
        y = self.rb_LastAtrbsFrame.origin.y;
    ...
    // 覆盖添加完后那一列;的最新高度
    self.rb_LastAtrbsFrame = atrbs.frame;
...
- (CGRect)maxHeightFrame
{
    __block CGRect maxHeightFrame = self.rb_LastAtrbsFrame;

    [self.rb_AtrbsArray enumerateObjectsUsingBlock:^(UICollectionViewLayoutAttributes * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (id<RBElementsFlowLayoutDelegate>)delegate
{
    return (id<RBElementsFlowLayoutDelegate>)self.collectionView.dataSource;
}
